...
| `200`                              | MCP 인증 통과. **core-api 쪽 키는 아직 검증되지 않았다** — 8-1의 4번을 해야 한다 |

### 8-3. 아직 없는 것

T-017 Scope의 `mcp:ping` 스크립트는 **이 브랜치에 구현돼 있지 않다**(사유는 태스크 파일의
Findings F-1). 그때까지 연결 확인은 위 두 방법으로 한다.
