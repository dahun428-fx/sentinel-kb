...
/**
 * **필수 절.** Acceptance 4·5는 필드 이름과 그 순서만 본다 — 필드 이름이 어차피 여러 번 나오는
 * 절(순서 근거, 저장 전 점검)은 통째로 지워도 둘 다 통과했다(뮤테이션 실측: 생존).
 * 그 두 절이 이 프롬프트에서 **가장 지우기 쉬우면서 가장 값이 큰** 부분이라 여기서 따로 잠근다.
 */
describe("필수 절이 통째로 사라지지 않는다", () => {
  it.each([
    // 순서를 왜 지켜야 하는지가 없으면 상대가 건너뛰자고 할 때 인터뷰어가 버틸 근거가 없다.
    ["질문 순서의 근거", "## 왜 이 순서인가"],
    // 기록 전 검색이 빠지면 중복 레코드가 남는다 — 조용한 손상이다.
    ["검색 먼저", "## 0. 먼저 검색한다"],
    ["관측 수집", "## 1. 관측된 사실을 모은다"],
    ["종류 판별", "## 2. 종류를 가른다"],
    ["incident 갈래", "## 3-A. incident 인터뷰"],
    ["divergence 갈래", "## 3-B. divergence 인터뷰"],
    ["제목·태그", "## 4. 제목·심각도·태그"],
    // 시크릿·길이 점검이 빠지면 새니타이저 한 겹에만 의존하게 된다.
    ["저장 전 점검", "## 5. 저장 전 점검"],
    ["시크릿 점검", "시크릿"],
    ["청킹 길이 규약", "3–10문장"],
    ["저장", "## 6. 저장"],
  ])("`%s` 절이 있다", (_label, heading) => {
    expect(POSTMORTEM_INTERVIEW_PROMPT).toContain(heading);
  });
});
