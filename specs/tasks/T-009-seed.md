...
- **F-6. ✅ 해소됨 (`feat/seed-reset-in-place`, 인간 승인).**
  ~~`--reset`의 필터는 멱등 키와 정확히 같아야 한다. `{project, title: {$in: 시드}}`.
  따라서 사용자가 시드와 똑같은 title로 기록한 레코드는 `--reset`이 지운다 — 멱등 키가
  "같은 레코드"라고 판정하는 것과 같은 기준이라 원리적으로 구분할 수 없다.~~

  **`--reset`이 더 이상 지우지 않는다 — 제자리 PATCH로 바뀌었다.** 소유권은 추측하지 않고
  `seedManifest` 컬렉션(`{project,title} → recordId`)에 적어 둔다. 장부에 적힌 레코드만
  `PATCH /v1/records/:id`로 갱신하므로 동명 사용자 레코드는 삭제도 갱신도 되지 않는다.
  통합 테스트가 시드와 **같은 제목**의 사용자 레코드를 넣고 `--reset` 후 도큐먼트 전체가
  (`updatedAt`까지) 불변임을 단언한다.

  **`seedBatch` 마커(=contracts 재개방, G3)를 택하지 않은 이유**: 그건 F-6만 풀고
  T-013의 `_id` 문제는 그대로 남긴다(재삽입하면 여전히 새 `_id`다). 장부는 records 컬렉션
  밖이라 `RecordSchema`의 `.strict()`를 건드리지 않으면서 **둘을 동시에** 푼다.

  **남은 잔여 위험(입양 경로)**: 이 변경 이전에 적재된 DB에는 장부가 없다. 그런 title은
  `{project,title}` 일치가 **정확히 1건일 때만** 시드의 것으로 입양하고 장부에 적는다
  (그러지 않으면 첫 `--reset`이 전부 새로 삽입해 제목이 중복된다). 따라서 "시드가 한 번도
  안 돌았는데 사용자가 시드 title로 먼저 기록한" 경우는 여전히 그 레코드를 갱신한다 —
  구분할 정보가 아직 존재하지 않는 구간이다. **2건 이상이면 찍지 않고
  `SEED_AMBIGUOUS_TITLE`로 던진다.** 한 번 시드를 돌리고 나면 이 구간은 닫힌다.

  **제자리 PATCH가 표현할 수 없는 것 두 가지** (둘 다 조용히 넘기지 않는다):
  - `type` 변경(`incident` ↔ `divergence`): `PatchRecordInput`에 `type`이 없고 PATCH 라우트가
    400으로 거부한다(specs/04: 종류 수정 불가). → `SEED_TYPE_CHANGED`로 던지고 사람에게
    결정을 넘긴다. 삭제 후 재삽입으로 우회하지 않는다 — 그게 `_id`를 버리는 동작이다.
  - **선택 필드 삭제**(`rootCause`·`prevention`을 시드 JSON에서 뺀 경우): PATCH는 필드를
    지우는 문법이 없으므로 옛 값이 남는다. 현재 시드 데이터에는 해당 사례가 없다. (F-12)
...
- `--reset`: 시드만 삭제하고 사용자 레코드 생존, chunks·jobs 함께 정리, 고아 0.
  **(당시 동작. `feat/seed-reset-in-place` 이후 `--reset`은 삭제하지 않고 제자리 PATCH한다 — F-6 참조.)**
