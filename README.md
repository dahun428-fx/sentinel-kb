...
| 지표 | 값 | 정의 |
| --- | --- | --- |
| 기록된 태스크 | 25개 (엔트리 28줄) | 분모 |
| 최종 상태 분포 | done 2 · GREEN 17 · PARTIAL 6 | 같은 태스크의 마지막 엔트리만 |
| 완결률 — 한 번에 | 24.0% | attempts=1 + 게이트 실패 0 + GREEN |
| 완결률 — 재시도 한도 안 | 72.0% | GREEN + attempts <= 3 |
| 완결률 — 결국 GREEN | 76.0% | 사람이 BLOCKED를 푼 경우 포함 |
| 게이트 실패(엔트리) | G1 1 · G2 7 · G3 0 · G4 0 · G5 5 · G6 0 | 한 번도 안 걸린 게이트도 0으로 싣는다 |
| attempts 합 | 56회 (최다 T-004 33.9%) | 루프 비용이 어디에 쏠렸는가 |
| 재시도 한도 초과 | T-004 | attempts > 3 |
| PLAN 3–7줄 기계 판정 | 26/27 (planLines 있는 엔트리 27/28) | planLines가 없으면 판정 불가 |
| filesRead 파일 수 | 최소 2 · 중앙값 12 · 최대 43 | 기록된 엔트리 27/28 |
